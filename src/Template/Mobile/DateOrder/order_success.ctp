...
<div class="wraper">
    <div class="date_pay_content flex flex_justify">
        <div class="date_pay_leftcon">
            <h1 class="pay_tips">您已支付成功！</h1>
            <p>注意提前预订好场地</p>
        </div>
        <div class="date_pay_rightcon">
            <a href="/date-order/order-detail/<?=$order_id?>" class="button btn_bg_active">查看订单</a>
...
$.util.asyLoadData({gurl: '/index/get-user-list/', page: curpage, tpl: '#user-list-tpl', id: '#user-list', key: 'users'});
setTimeout(function () {
    $(window).on("scroll", function () {
        $.util.listScroll('place-list', function () {
            //window.holdLoad = false;  //打开加载锁  可以开始再次加载
            $.util.asyLoadData({gurl: '/index/get-user-list/', page: curpage,
