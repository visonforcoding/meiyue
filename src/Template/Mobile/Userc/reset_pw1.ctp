    ...
    <div class="foget-pwd-box">
        <div class="items">
            <div class="flex items-box">
                <span class="prenum">+86</span><input type="number" id="phone" type="text" placeholder="请输入手机号" value="<?= isset($user)?$user->phone:''?>" <?= isset($user)?'disabled':''?>/>
            </div>
        </div>
        <div class="items">
            <div class="flex  flex_justify items-box">
                <input id="vcode" type="text" placeholder="请输入验证码" class="valinum" />
                <span id="gcode" class="getvolid <?= isset($user)?'':'disabled'?>">获取验证码</span>
            </div>
        </div>
